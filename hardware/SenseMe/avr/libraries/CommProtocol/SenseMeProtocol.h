/**
* \defgroup SenseMeProtocol Driver for the SenseMe Serial Terminal
* 
* @{
*/
...
/**
* \file 
* 		SenseMe Protocol driver
* \author
* 		SenseMe team: support@SenseMe.org
*/

#ifndef __SENSEMEPROTOCOL_H__
#define __SENSEMEPROTOCOL_H__

#include <Arduino.h>
#include <SenseMe.h>

#include <SenseMeLEDs.h>
...
// Version
#define VERSION		30	// SenseMeV3_0_SKETCH_BASIC 
...
#define TIME_STAMP	2.05  //ms
#define MIN_TIME	20    //ms



// Structures
struct SenseMePackage {
    byte packageType;
	byte commandID;
	long packageID;
	byte ack;
...
class SenseMeProtocolClass 
{
	protected:
		SenseMeLEDsClass* SenseMeLEDs;
		SenseMeThermistorClass* SenseMeThermistor; 
		SenseMeAccelerometerClass* SenseMeAccelerometer; 
		SenseMeMagnetometerClass* SenseMeMagnetometer;
		SenseMeLightClass* SenseMeLight;
		SenseMeButtonClass* SenseMeButton;
		SenseMeIRClass* SenseMeIR;
	...
	public:
		SenseMeProtocolClass();
		~SenseMeProtocolClass();
		void begin(SenseMeLEDsClass *eledsc, 
		...
		int parsePackageHeader(byte communicationChannel, byte inNrVals, long *inVals, struct SenseMePackage *outPackage, byte *outHeaderSize);
		int sendPackage(byte inNrVals, byte inCommandID, long *inVals);
		int sendPackage(struct SenseMePackage *engPackage, byte inNrVals, byte inCommandID, long *inVals);
		int setLED(struct SenseMePackage *engPackage, byte nrVals, long *inVals);
		int setLEDs(struct SenseMePackage *engPackage, byte nrVals, long *inVals);
		int setGetStatus(struct SenseMePackage *engPackage, byte nrVals, long *inVals, uint8_t setGet);
		int setGetIR(struct SenseMePackage *engPackage, byte nrVals, long *inVals, uint8_t setGet);
		int getVersion(struct SenseMePackage *engPackage);
		int getSensor(struct SenseMePackage *engPackage, int sensorType, byte nrVals, long *inVals); 
		int getButton(struct SenseMePackage *engPackage, byte nrVals, long *inVals);
...
extern SenseMeProtocolClass SenseMeProtocol;
